...
function renderGate(gate: Gate): string {
	...
	const expect = typeof gate.expect === "string" && gate.expect.length > 0
		? ` = ${gate.expect}`
		: "";
 ...
 * Returns null once every task is done, so completed work stops consuming context.
 */
export function taskContextFromDb(db: Db): string | null {
	...
	if (failed.length > 0) {
		lines.push(`Blocked: ${failed.map((task) => `${task.title} (${task.id})`).join(", ")}`);
	}
