...
describe("Notes daemon operations", () => {
	it("exposes authenticated capture, inbox, consumption, promotion, and archive behavior", async () => {
		...
		expect(EXPECTED_OPERATION_NAMES).toEqual(
			expect.arrayContaining(["notes.capture", "notes.list", "notes.show", "notes.consume", "notes.promote", "notes.archive"]),
		);
		const note = await client.call<Record<string, unknown>, { id: string; status: string; subtype: string }>("notes.capture", {
			body: "Investigate the retry policy",
		...
		expect(
			await client.call<Record<string, unknown>, Artifact>("notes.consume", {
				id: note.id,
				project_root: PROJECT,
				actor: "agent",
				source: "test",
		...
		const target = await client.call<Record<string, unknown>, { id: string }>("docs.create", {
			title: "Retry research",
			subtype: "research",
		});
		expect(
			await client.call<Record<string, unknown>, Artifact>("notes.promote", {
				id: note.id,
				target_id: target.id,
				project_root: PROJECT,
