...
describe("session-scoped Task Focus survives interleaved multi-session work without any session observing or mutating another's Focus", () => {
	it("three sessions each create, focus, pause, unpause, and refocus, interleaved step by step", async () => {
		...
		const bobStillActive = (await service.execute("tasks.focused", {
			project_root: PROJECT_ROOT,
			session_id: "session-bob",
		})) as FocusState | null;
		expect(bobStillActive?.status).toBe("active");
		const aliceNowPaused = (await service.execute("tasks.focused", {
			project_root: PROJECT_ROOT,
			session_id: "session-alice",
		})) as FocusState | null;
		expect(aliceNowPaused?.status).toBe("paused");
		const carolStillActive = (await service.execute("tasks.focused", {
			project_root: PROJECT_ROOT,
		...
		const carolRefocused = (await service.execute("tasks.active", {
			project_root: PROJECT_ROOT,
			session_id: "session-carol",
		})) as CliArtifact | null;
		...
		const aliceResumed = (await service.execute("tasks.focused", {
			project_root: PROJECT_ROOT,
			session_id: "session-alice",
		})) as FocusState | null;
		...
		const bobUnaffected = (await service.execute("tasks.active", {
			project_root: PROJECT_ROOT,
			session_id: "session-bob",
		})) as CliArtifact | null;
		expect(bobUnaffected?.id).toBe(tasksBySession["session-bob"]!.id);
		const carolUnaffected = (await service.execute("tasks.active", {
			project_root: PROJECT_ROOT,
			session_id: "session-carol",
		})) as CliArtifact | null;
		...
		const aliceStillThere = (await service.execute("tasks.active", {
			project_root: PROJECT_ROOT,
			session_id: "session-alice",
		})) as CliArtifact | null;
		expect(aliceStillThere?.id).toBe(tasksBySession["session-alice"]!.id);
		const carolStillThere = (await service.execute("tasks.active", {
			project_root: PROJECT_ROOT,
			session_id: "session-carol",
		})) as CliArtifact | null;
	...
	it("a caller that omits session_id entirely uses the shared global scope, isolated from every explicitly-scoped session", async () => {
		...
		const scopedActive = (await service.execute("tasks.active", {
			project_root: PROJECT_ROOT,
			session_id: "session-dedicated",
		})) as CliArtifact | null;
