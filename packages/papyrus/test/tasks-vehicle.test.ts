...
describe("registerTasksVehicleOperations (wired through createPapyrusService)", () => {
	it("registers exactly one honest VehicleOperation per real tasks.* action, never an action-dispatch schema -- excluding the two system-maintenance operations the removed tool never exposed either", () => {
		...
		const names = registry.manifest().operations.map((op: VehicleManifestOperation) => op.name).filter((name: string) => name.startsWith("tasks.")).sort();
		expect(names).toEqual([
			"tasks.active", "tasks.assign_project", "tasks.cancel", "tasks.cancel_subtree", "tasks.claim",
			"tasks.clear_focus", "tasks.complete", "tasks.contain", "tasks.context", "tasks.create", "tasks.depend",
			"tasks.event_feed", "tasks.focus", "tasks.focused", "tasks.graph", "tasks.heartbeat_lease", "tasks.history",
			"tasks.lease", "tasks.list", "tasks.pause", "tasks.plan", "tasks.reject", "tasks.release_lease",
			"tasks.retry", "tasks.run_gates", "tasks.scope", "tasks.set_checklist", "tasks.set_gates", "tasks.set_scope",
			"tasks.show", "tasks.start", "tasks.submit", "tasks.uncontain", "tasks.undepend", "tasks.unpause",
			"tasks.update",
		]);
	...
	it("create requires project_root explicitly -- no ambient cwd server-side", async () => {
		...
		const created = (await registry.invoke("tasks.create", 1, { title: "Ship the feature", project_root: PROJECT }, PERMS)) as { id: string; title: string };
	...
	it("create resolves parent_name and depends_on_names server-side", async () => {
		...
		const child = (await registry.invoke("tasks.create", 1, { title: "Child", project_root: PROJECT, parent_name: "Parent", depends_on_names: ["Dependency"] }, PERMS)) as { id: string };
		const graph = (await registry.invoke("tasks.graph", 1, { project_root: PROJECT }, PERMS)) as { nodes: Array<{ task: { id: string }; parentIds: string[]; dependencyIds: string[] }> };
	...
	it("start/submit/complete drive the full lifecycle, and complete's output carries a model-facing content summary", async () => {
		...
		const completion = (await registry.invoke("tasks.complete", 1, { id: created.id }, PERMS)) as { completed: boolean; content?: Array<{ type: string; text: string }> };
	...
	it("plan's output carries a layered execution-plan content summary", async () => {
		...
		const plan = (await registry.invoke("tasks.plan", 1, { project_root: PROJECT }, PERMS)) as { layers: string[][]; content?: Array<{ type: string; text: string }> };
	...
	it("run_gates' output carries a pass/fail content summary", async () => {
		...
		const result = (await registry.invoke("tasks.run_gates", 1, { id: created.id }, PERMS)) as { gates: unknown[]; content?: Array<{ type: string; text: string }> };
	...
	it("cancel_subtree's output carries a canceled/skipped count content summary", async () => {
		...
		const outcome = (await registry.invoke("tasks.cancel_subtree", 1, { id: parent.id }, PERMS)) as { canceled: string[]; content?: Array<{ type: string; text: string }> };
	...
	it("context's output carries the same reconciliation summary as its own context field, as content", async () => {
		...
		const result = (await registry.invoke("tasks.context", 1, { project_root: PROJECT }, PERMS)) as { context: string | null; content?: Array<{ type: string; text: string }> };
	...
	it("depend resolves dependency_name in a different project by widening scope once, when no explicit scope was pinned", async () => {
		...
		const other = (await registry.invoke("tasks.create", 1, { title: "Companion task", project_root: "/tmp/other-project" }, PERMS)) as { id: string };
		const updated = (await registry.invoke("tasks.depend", 1, { id: from.id, dependency_name: "Companion task", project_root: PROJECT }, PERMS)) as { id: string };
		...
		const graph = (await registry.invoke("tasks.graph", 1, { project_root: PROJECT, scope: "all" }, PERMS)) as { nodes: Array<{ task: { id: string }; dependencyIds: string[] }> };
	...
	it("does not widen when the caller pinned an explicit scope, so a genuine not-found stays a real error", async () => {
		...
		const rejection = await registry.invoke("tasks.depend", 1, { id: from.id, dependency_name: "Elsewhere", project_root: PROJECT, scope: "project" }, PERMS).catch((error: unknown) => error);
		expect(rejection).toBeInstanceOf(Error);
	...
	it("depend/undepend resolve dependency_name server-side and are idempotent", async () => {
		...
		const graph = (await registry.invoke("tasks.graph", 1, { project_root: PROJECT }, PERMS)) as { nodes: Array<{ task: { id: string }; dependencyIds: string[] }> };
	...
	it("focus/pause/unpause/clear_focus authorize their write via principal.claims.sessionId/sessionSecret, never a model-visible input field", async () => {
		...
		const rejection = await registry.invoke("tasks.focus", 1, { id: created.id }, {
			...PERMS,
		...
		const focused = (await registry.invoke("tasks.focus", 1, { id: created.id }, {
			...PERMS,
			principal: { id: "pi-papyrus", claims: { sessionId: "session-1", sessionSecret: secret } },
		})) as { id: string };
		expect(focused.id).toBe(created.id);

		const paused = (await registry.invoke("tasks.pause", 1, {}, {
			...PERMS,
			principal: { id: "pi-papyrus", claims: { sessionId: "session-1", sessionSecret: secret } },
	...
	it("claim/heartbeat_lease/release_lease/lease resolve by name and round-trip a real lease", async () => {
		...
		const claimed = (await registry.invoke("tasks.claim", 1, { name: "Lease me", project_root: PROJECT, owner: "worker-1" }, PERMS)) as { owner: string; token: string };
		...
		const released = (await registry.invoke("tasks.release_lease", 1, { name: "Lease me", project_root: PROJECT, owner: "worker-1", token: claimed.token }, PERMS)) as { released: boolean };
	...
	it("real incident: tasks.update's own schema never declares gates, and a gates field riding along on it has zero effect -- set_gates is the one true way to change gates", async () => {
		const { registry, service } = harness();
		const updateSchema = registry.manifest().operations.find((op: VehicleManifestOperation) => op.name === "tasks.update")!.inputSchema as { properties: Record<string, unknown> };
		expect(updateSchema.properties).not.toHaveProperty("gates");

		const created = (await registry.invoke("tasks.create", 1, { title: "Gate me", project_root: PROJECT, gates: [{ type: "command", target: "echo original" }] }, PERMS)) as { id: string };
		...
		await registry.invoke("tasks.update", 1, { id: created.id, body: "new body", gates: [{ type: "command", target: "echo should-be-ignored" }] }, PERMS);
	...
	it("remove/restore go through the shared kind-agnostic artifact.* operations, not a tasks-namespaced duplicate", async () => {
		const { registry, service } = harness();
		const created = (await registry.invoke("tasks.create", 1, { title: "Trash me", project_root: PROJECT }, { permissions: ["tasks:read", "tasks:write", "artifact:read", "artifact:write"] })) as { id: string };
		await registry.invoke("artifact.remove", 1, { id: created.id }, { permissions: ["tasks:read", "tasks:write", "artifact:read", "artifact:write"] });
