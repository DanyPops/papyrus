...
interface RegisteredDiscussTool {
	execute: ToolExecute;
...
function discussTool(): RegisteredDiscussTool {
	...
	const fakeApi = {
		registerTool: (tool: RegisteredDiscussTool & { name: string }) => tools.set(tool.name, tool),
...
function fakeCtx(overrides: Partial<ExtensionContext> = {}): ExtensionContext {
	return {
		cwd: "/home/dpopsuev/Projects/papyrus",
		hasUI: true,
		ui: { select: async () => undefined, input: async () => undefined, notify: () => {}, custom: async () => undefined } as any,
		...overrides,
	} as ExtensionContext;
}
...
describe("discuss tool: live:true synchronous ask, on top of the normal async round", () => {
	it("live:false (default): open just records the round and returns immediately, no ctx.ui touched", async () => {
		...
		mockCalls({
			"discuss.open": () => ({
				discussion: OPENED_DISCUSSION,
				rounds: [{ id: 1, discussionId: "d1", roundNumber: 1, actor: "assistant", content: "hi", occurredAt: "x" }],
			}),
		});
		const ctx = fakeCtx({
			ui: {
		...
		const result = await execute(
			"id1",
			{ action: "open", title: "Ship or not?", actor: "assistant", content: "hi" },
			undefined,
			undefined,
			ctx,
		);
	...
	it("live:true with no pending options: prompts freeform via ctx.ui.input and records the live answer as a second round", async () => {
		...
		const calls = mockCalls({
			...
			"discuss.reply": (input: any) => ({
				discussion: { ...OPENED_DISCUSSION, title: "Ship or not?" },
				rounds: [{ id: 2, discussionId: "d1", roundNumber: 2, actor: "human", content: input.content, occurredAt: "x" }],
			}),
		});
		...
		const result = await execute(
			"id1",
			{ action: "open", title: "Ship or not?", actor: "assistant", content: "Should we ship Friday?", live: true },
			undefined,
			undefined,
			ctx,
		);
		// Regression: a bare "Reply to <title>:" prompt with no visible question content left the
		// human with nothing to answer. The round's own real content is now the question itself
		// (dialog fallback doesn't render the subtitle -- that's an AskComponent-only affordance).
	...
	it("live:true with a pending structured choice: renders the picker (ctx.ui.select for single mode), never the freeform input", async () => {
		...
		const calls = mockCalls({
			...
			"discuss.reply": (input: any) => ({
				discussion: withPending,
				rounds: [
					{ id: 2, discussionId: "d1", roundNumber: 2, actor: "human", content: input.content, occurredAt: "x", selected: input.selected },
				],
			}),
		});
	...
	it("live:true degrades silently to the async result when there is no interactive UI (RPC/headless)", async () => {
		...
		const result = await execute(
			"id1",
			{ action: "open", title: "Ship or not?", actor: "assistant", content: "q", live: true },
			undefined,
			undefined,
			ctx,
		);
	...
	it("live:true where the human cancels the prompt: keeps the recorded round, does not fabricate a reply", async () => {
		...
		const calls = mockCalls({
			"discuss.reply": () => ({
				discussion: OPENED_DISCUSSION,
				rounds: [{ id: 2, discussionId: "d1", roundNumber: 2, actor: "assistant", content: "q2", occurredAt: "x" }],
			}),
		});
		const ctx = fakeCtx({
			ui: { select: async () => undefined, input: async () => undefined, notify: () => {}, custom: async () => undefined } as any,
		...
		const result = await execute(
			"id1",
			{ action: "reply", name: "Ship or not?", id: "d1", actor: "assistant", content: "q2", live: true },
			undefined,
			undefined,
			ctx,
		);
	...
	it("live:true streams a heartbeat onUpdate before blocking on the human, so a slow human response isn't mistaken for a dead tool call", async () => {
		...
		const ctx = fakeCtx({
			ui: { select: async () => undefined, input: async () => "Yes", notify: () => {}, custom: async () => undefined } as any,
		});
		await execute(
			"id1",
			{ action: "open", title: "Ship or not?", actor: "assistant", content: "q", live: true },
	...
	it("sends no option_descriptions at all when every option is a bare string (no tradeoff worth conveying)", async () => {
		...
		await execute(
			"id1",
			{ action: "open", title: "Ship or not?", actor: "assistant", content: "q", options_mode: "single", options: ["Yes", "No"] },
			undefined,
			undefined,
			fakeCtx(),
		);
	...
	it("live:true renders pending option descriptions in the picker", async () => {
		...
		const withPending = {
			...
			extra: {
				discussion: {
					state: "active",
					roundCount: 1,
					pendingOptions: ["Ship Friday", "Slip to Monday"],
					pendingOptionsMode: "single",
					pendingOptionDescriptions: ["Fast, riskier", "Slower, safer"],
				},
			},
		};
		mockCalls({
		...
		const ctx = fakeCtx({
			ui: {
				select: async () => {
					throw new Error("must not fall back to dialogs");
				},
				input: async () => {
					throw new Error("must not fall back to dialogs");
				},
