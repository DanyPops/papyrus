...
describe("generic mutation event log — explicit migration", () => {
	it("adds artifact_events when migrating an existing schema 6 database forward", () => {
		...
		expect(migrateDb(db)).toEqual({ from: 6, to: 18, applied: ["artifact-event-log", "task-focus-session-scope", "graph-projection-protocol", "docs-rules-skills-project-scope", "log-domain", "remove-discourse", "session-identity", "artifact-trash", "discuss-native", "discuss-options", "discussion-task-kind", "playbook-kind"] });
