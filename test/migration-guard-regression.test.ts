...
describe("migrateDb guard regression: schema versions 8, 9, 10 must have a real migration path", () => {
	it("migrates from schema version 10 (today's real, live production shape) to current", () => {
		...
		expect(result.applied).toEqual(["log-domain", "remove-discourse", "session-identity", "artifact-trash"]);
	...
	it("migrates from schema version 9 to current (the version the old guard would have rejected first)", () => {
		...
		expect(result.applied).toEqual(["docs-rules-skills-project-scope", "log-domain", "remove-discourse", "session-identity", "artifact-trash"]);
		db.close();
	});
	...
	it("migrates from schema version 8 to current", () => {
		...
		expect(result.applied).toEqual(["graph-projection-protocol", "docs-rules-skills-project-scope", "log-domain", "remove-discourse", "session-identity", "artifact-trash"]);
		db.close();
	});
