...
describe("append-only task lifecycle history", () => {
	...
	it("migrates v2 explicitly without fabricating history for existing tasks", () => {
		...
		expect(migrateDb(db)).toEqual({ from: 2, to: 10, applied: ["task-history", "task-project-scope", "task-focus-continuation", "discourse-context-mesh", "artifact-event-log", "task-focus-session-scope", "graph-projection-protocol", "docs-rules-skills-project-scope"] });
