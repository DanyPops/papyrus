...
describe("task lifecycle schema migration", () => {
	it("migrates lifecycle statuses and preserves one deterministic active focus", () => {
		...
		expect(migrateDb(db)).toEqual({ from: 1, to: 13, applied: ["task-lifecycle-and-focus", "task-history", "task-project-scope", "task-focus-continuation", "discourse-context-mesh", "artifact-event-log", "task-focus-session-scope", "graph-projection-protocol", "docs-rules-skills-project-scope", "log-domain", "remove-discourse", "session-identity"] });
