...
describe("task lifecycle schema migration", () => {
	it("migrates lifecycle statuses and preserves one deterministic active focus", () => {
		...
		expect(migrateDb(db)).toEqual({ from: 1, to: 2, applied: ["task-lifecycle-and-focus"] });
		...
		expect(db.prepare("SELECT task_id FROM task_focus WHERE scope = 'global'").get()).toEqual({ task_id: "newer-active" });
