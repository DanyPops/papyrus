...
describe("task project scope migration", () => {
	it("keeps existing tasks explicitly unscoped instead of guessing a project", () => {
		...
		db = openDb(path);
		expect(migrateDb(db)).toEqual({ from: 3, to: 13, applied: ["task-project-scope", "task-focus-continuation", "discourse-context-mesh", "artifact-event-log", "task-focus-session-scope", "graph-projection-protocol", "docs-rules-skills-project-scope", "log-domain", "remove-discourse", "session-identity"] });
